...
if (inputedExit)
{
    Console.WriteLine("Вы завершили работу с прогрммой. Программа будет закрыта после нажатия на любую клавишу");
}
else
{
    ...
    {
        CompareMinAndMax(studentsWeight); //Вынес код в отдельную функцию, чтобы было более читабельно и понятно читать код
...
void CompareMinAndMax(List<int> studentsWeight)
{
    ...
    {
        Console.WriteLine("Вес самого тяжелого больше чем в 2 раза больше наименьшего веса");
    }
    else
    {
        Console.WriteLine("Вес самого тяжелого не в 2 раза больше наименьшего веса");
    }
}